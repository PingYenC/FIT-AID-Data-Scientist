...
6. Result Storing and Conclusion
   * Although I implemented oversampling to balance the data, the unbalanced nature of the original dataset restricted the models' capabilities. Oversampling is a method of enhancing models' training process but a good result is not guaranteed. Hence, I would say that the models were doing their best to find the relationships within the dataset and the best they could do was up to 0.66 f1 score. Better models may be trained by using balanced data (without manually balancing).
